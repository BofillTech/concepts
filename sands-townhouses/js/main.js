...
(function(){
  'use strict';
  var nav=document.getElementById('nav'), bbar=document.getElementById('bbar');
  ...
  if(t&&links){t.addEventListener('click',function(){var o=links.classList.toggle('is-open');t.setAttribute('aria-expanded',o);});
  ...
  var slides=document.querySelectorAll('.hero__slide'),dots=document.querySelectorAll('#heroDots button'),hi=0,htimer;
  ...
  if(slides.length>1 && !matchMedia('(prefers-reduced-motion: reduce)').matches){htimer=setInterval(function(){go(hi+1);},5500);
  ...
  if('IntersectionObserver' in window){var io=new IntersectionObserver(function(es){es.forEach(function(en){if(en.isIntersecting){en.target.classList.add('in');io.unobserve(en.target);}});},{threshold:.12,rootMargin:'0px 0px -8% 0px'});
  ...
  // contact form -> compose email (no page reload)
  ...
  if(cf){cf.addEventListener('submit',function(e){e.preventDefault();
    ...
    var subj='Website inquiry from '+(v('name')||'a guest');
    var body='Name: '+v('name')+'\nEmail: '+v('email')+'\nPhone: '+v('phone')+'\n\n'+v('message');
    window.location.href='mailto:sandsbeachcondosoob@gmail.com?subject='+encodeURIComponent(subj)+'&body='+encodeURIComponent(body);
  });}
})();
