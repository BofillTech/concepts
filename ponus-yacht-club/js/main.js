/* Ponus Yacht Club — concept by Bofill Technologies */
(function () {
  'use strict';
  ...
  /* Masthead — single orchestrated staggered-reveal on load */
  var mast = document.querySelector('.mast');
  if (mast) {
    if ('requestAnimationFrame' in window) {
      requestAnimationFrame(function () {
        requestAnimationFrame(function () { mast.classList.add('is-loaded'); });
      });
    } else {
      mast.classList.add('is-loaded');
    }
  }
  ...
  /* Image fallback — failed photos hide so the frame still holds its shape */
